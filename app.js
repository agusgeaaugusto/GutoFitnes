...
let state = { enfoque: select.value, day: "Lunes" };

function saveKey(){ return `gf.v1.${state.enfoque}.${state.day}`; }
function loadProgress(){ try{ return JSON.parse(localStorage.getItem(saveKey())||"{}"); }catch(e){ return {}; } }
function saveProgress(p){ localStorage.setItem(saveKey(), JSON.stringify(p)); }
...
function card(ex, idx){
  ...
  <article class="card">
    ...
    <div class="body">
      ...
      <div class="meta">${ex.time || ""}${ex.tips ? " · "+ex.tips:""}</div>
      <div class="row">
...
function render(){
  const plan = window.PLANES[state.enfoque]?.[state.day] || [];
  list.innerHTML = plan.map((ex,i)=>card(ex,i)).join("") || "<p>No hay ejercicios.</p>";
  // eventos
  $$(".play", list).forEach(b=> b.addEventListener('click', e=>{
    const src = e.currentTarget.dataset.vid; openModal(src);
  }));
  $$(".kg", list).forEach(inp => inp.addEventListener('input', e=>{
    const idx = +e.currentTarget.dataset.idx;
    const p = loadProgress(); p[idx] = p[idx] || {}; p[idx].kg = e.currentTarget.value; saveProgress(p);
  }));
  $$(".toggle", list).forEach(btn => btn.addEventListener('click', e=>{
    const idx = +e.currentTarget.dataset.idx;
    const p = loadProgress(); p[idx] = p[idx] || {}; p[idx].done = !p[idx].done; saveProgress(p); render();
  }));
}

select.addEventListener('change', e=>{
  state.enfoque = e.target.value; render();
});
